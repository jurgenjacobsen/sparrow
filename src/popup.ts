...
async function loadData() {
  ...
  settings = { ...DEFAULT_SETTINGS, ...(data.settings as Partial<Settings>) };
  loadSettingsUI();
}
...
function loadSettingsUI() {
  (document.getElementById('desktopNotifications') as HTMLInputElement).checked =
    settings.desktopNotifications;
  (document.getElementById('notificationSounds') as HTMLInputElement).checked =
    settings.notificationSounds;
}

async function saveSettings() {
  settings.desktopNotifications = (
    document.getElementById('desktopNotifications') as HTMLInputElement
  ).checked;
  settings.notificationSounds = (
...
function openEditReminder(id: string) {
...
function renderAddReminderRow() {
  ...
  const units: Array<'minutes' | 'hours' | 'days' | 'weeks'> = [
    'minutes',
    'hours',
    'days',
    'weeks',
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    const unit = unitSelect.value as Reminder['unit'];
    const repeatType = (
      repeatContainer.querySelector('input[type="radio"]:checked') as HTMLInputElement | null
    )?.value as Reminder['repeatType'];
